...
\begin{document}
	...
	\begin{multicols}{2}
		...
		Each compute node creates blank a 'report card' about all the territories that it owns and passes it to the other compute nodes in order to fill it in.
		The report card itself is another matrix slice similar to $\emph{edgeActivity}$, where $\emph{reportCard}[x][y] = T$ represents the fact that $
